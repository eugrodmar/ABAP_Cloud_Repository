CLASS zcl_torneo_tenis_17 DEFINITION
PUBLIC
  FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    METHODS:
      ...
      registrar_jugador
        IMPORTING
          io_jugador TYPE REF TO zcl_jugador_base_17
        RAISING
          zcx_tenis_error_17,
      ...
      get_lider
        RETURNING VALUE(ro_lider) TYPE REF TO zcl_jugador_base_17
        RAISING
                  zcx_tenis_error_17,
  ...
  PRIVATE SECTION.
    ...
    DATA mt_jugadores TYPE STANDARD TABLE OF REF TO zcl_jugador_base_17
         WITH EMPTY KEY.
    DATA mt_resultados TYPE ty_resultados.

    METHODS buscar_jugador
      IMPORTING iv_nombre         TYPE string
      RETURNING VALUE(ro_jugador) TYPE REF TO zcl_jugador_base_17.

ENDCLASS.

CLASS zcl_torneo_tenis_17 IMPLEMENTATION.
  ...
  METHOD registrar_jugador.
    DATA lo_existe TYPE REF TO zcl_jugador_base_17.
    lo_existe = buscar_jugador( io_jugador->get_nombre( ) ).
    IF lo_existe IS NOT INITIAL.
      RAISE EXCEPTION TYPE zcx_tenis_error_17
        EXPORTING
          iv_mensaje = |Jugador { io_jugador->get_nombre( ) } ya registrado|.
    ENDIF.
  ...
  METHOD registrar_partido.
    DATA lo_ganador  TYPE REF TO zcl_jugador_base_17.
    DATA lo_perdedor TYPE REF TO zcl_jugador_base_17.
    ...
    IF lo_ganador IS INITIAL.
      RAISE EXCEPTION TYPE zcx_tenis_error_17
        EXPORTING
          iv_mensaje = |Jugador { iv_nombre_ganador } no encontrado|.
    ENDIF.
    IF lo_perdedor IS INITIAL.
      RAISE EXCEPTION TYPE zcx_tenis_error_17
        EXPORTING
          iv_mensaje = |Jugador { iv_nombre_perdedor } no encontrado|.
    ENDIF.
    ...
    lo_perdedor->get_derrotas( ).

    DATA ls_res TYPE zcl_torneo_tenis_17=>ty_resultado.
  ...
  METHOD get_lider.
    ...
    LOOP AT mt_jugadores INTO DATA(lo_j).
      DATA(lv_pts) = lo_j->zif_puntuable_17~calcular_puntos( ).
    ...
    IF ro_lider IS INITIAL.
      RAISE EXCEPTION TYPE zcx_tenis_error_17
        EXPORTING
          iv_mensaje = 'No hay jugadores en el torneo'.
    ENDIF.
  ENDMETHOD.

  METHOD get_clasificacion.
    DATA lt_pares TYPE STANDARD TABLE OF
         REF TO zcl_jugador_base_17 WITH EMPTY KEY.

    lt_pares = mt_jugadores.

*    SORT lt_pares BY table_line->zif_puntuable_17~calcular_puntos( ) DESCENDING.

    DATA lv_pos TYPE i VALUE 1.
    LOOP AT lt_pares INTO DATA(lo_j).
      DATA(lv_linea) = |{ lv_pos }. { lo_j->zif_puntuable_17~get_resumen( ) }|.
